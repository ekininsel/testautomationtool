	...
	private static void classGenerator(String classDirectory, CompilationUnit cu) throws Exception {
		...
		for (int i = 0; i < array.size(); i++) {
			FieldSpec object = FieldSpec
					.builder(findClassName, getClassName(classDirectory).toString().toLowerCase())
			...
			MethodSpec testGenerator = MethodSpec.methodBuilder("test" + array.get(i))
					.addAnnotation(Test.class).addModifiers(Modifier.PUBLIC).build();
			...
			TypeSpec generatedTestClass = TypeSpec.classBuilder(getClassName(classDirectory) + "TestClass")
					.addField(object).addMethod(beforeGeneration).addMethod(constructorSpec).addMethod(testGenerator)
					.addMethod(parameterizedData).addModifiers(Modifier.PUBLIC).build();
